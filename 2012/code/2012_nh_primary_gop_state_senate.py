...
urls=['http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28552',
      'http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28553',
      'http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28554',
      'http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28555',
      'http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28556',
      'http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28557',
      'http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28558',
      'http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28559',
      'http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28560',
      'http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=28561'
      ]
...
import csv
csvfile=open(rep_dir+'/20120911__nh__republican__primary__state__senate__town.csv','wb')
            ...
            if(header_row==1):
                ...
                for col in range(1,ws.ncols):
                    ...
                    if('Scatter' in value):
                        ...
                        cols_dict[col]=candidate
                        del cols_dict[col-1]
                    elif('write-in' in value or 'w-in' in value):
                        candidate=re.search('.*(?=\sw)',value).group(0)
                        candidate_dict[candidate]=dict()
                        candidate_dict[candidate]['Party']='IND'
                        cols_dict[col]=candidate
                    elif(value!='' and not value.isspace() and '(' not in value):
...
#                            candidate_dict[candidate]['Party']='IND'
            ...
            if('totals' in str(ws.cell(row,0).value).lower()):
                ...
                for col in cols_dict:
                    candidate=cols_dict[col]
                    if(col==winning_col and candidate_dict[candidate]['Party']=='R'):
