...
url='http://sos.nh.gov/WorkArea/DownloadAsset.aspx?id=26663'
...
candidate_dict={'Bass':{'name':'Charles Bass' ,
                ...
                'Kuster' :{'name':'Ann McLane Kuster',
                           'party':'D',
                           'winner':False},
                'Beloin' :{'name':'Gerard Beloin',
                          'party':'R',
                          'winner':False},
                'Dean' :{'name':'Will Dean',
                          'party':'R',
                          'winner':False},
                'Dziedzic' :{'name':'Miroslaw Dziedzic',
                          'party':'R',
                          'winner':False},
                'Lamare' :{'name':'Dennis Lamare',
                          'party':'R',
                          'winner':False},
    ...
    for row in range(ws.nrows):
        if(start_flag==1 and stop_flag==0):
            if('Totals' in ws.cell(row,0).value):
                ...
                town=re.sub('\*','',town)
                if(town=='At. & Gil Ac. Gt' or 
                   town=="Atkinson & Gilmanton Academy Grant"):
                    town='At. & Gil. Academy Grant'
                if(town=='Thomp. and Mes\'s Pur.' or 
                   town=="Thompson & Meserve's Purchase"):
...
import csv
csvfile=open(rep_dir+'/20120911__nh__republican__primary__house__2__town.csv','wb')
